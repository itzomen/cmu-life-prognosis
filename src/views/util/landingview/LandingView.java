    ...
    public void start() {
        while (true) {
            ...
            if (s.equals("1")) {
                ...
                vout = vops.performCheck("Enter UUID provided by admin or  * to back","Invalid UUID. Enter again or * to go back",
                        authenticationController::uuidValid, false, pDisplay);
                if (!vout.isValid())
                    continue;
                uuid = vout.getInput();

                vout = vops.performCheck("Enter First Name or *","Invalid Name: name shouldn't contain a number. Enter again or * to go back",
                ...
                vout = vops.performCheck(
                        "Enter Last Name","Invalid last Name: name shouldn't contain a number. Enter again or * to go back",
                        authenticationController::fNameValid, false, pDisplay);
                if (!vout.isValid())
                    continue;
                lName = vout.getInput();
                ...
                vout = vops.performCheck(
                        "Enter password ","Invalid password(length: 8, numbers, uppercase and lowercase letters). Enter again or * to go back",
                        authenticationController::passwordValid, true, pDisplay);
                if (!vout.isValid())
                    continue;
                password = vout.getInput();

                if(!vops.checkPassConfirmation(password, pDisplay)) continue;
